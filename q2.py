...
#Optional CLI arg: python q2.py bible
tag = sys.argv[1].lower() if len(sys.argv) > 1 else None

pool = QUOTES
if tag:
    filtered = [q for q in QUOTES if q.get("tag", "").lower() == tag]
    #if tag has no matches, filtered will be empty
    pool = filtered or QUOTES

n = len(pool)
index = today.toordinal() % n 
quote = pool[index]
print(f"{today} — {quote['text']} — {quote['author']} [{quote.get('tag','')}]")